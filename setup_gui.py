...
"""
py2app/py2exe build script for the PyQt4 interface to CuPED.
...
# Prepare lists of all the icons used by the GUI.
circular_icons = \
   ['bin/icons/circular/add.png', \
    'bin/icons/circular/arrow_down.png', \
    'bin/icons/circular/arrow_up.png', \
    'bin/icons/circular/delete.png', \
    'bin/icons/circular/edit.png', \
    'bin/icons/circular/heart_blue.png', \
    'bin/icons/circular/heart_green.png', \
    'bin/icons/circular/information.png', \
    'bin/icons/circular/minus.png', \
    'bin/icons/circular/star_green.png', \
    'bin/icons/circular/star_red.png']
...
tango_icons = \
   ['bin/icons/tango/document-new.png', \
    'bin/icons/tango/document-open.png', \
    'bin/icons/tango/document-save.png', \
    'bin/icons/tango/folder-open.png']

# Data files to be bundled with the application.
data_files = templates + [
    ('bin/icons/circular', circular_icons),
    ('bin/icons/cuped', cuped_icons),
    ('bin/icons/tango', tango_icons),
]
...
if sys.platform == 'darwin':
    ...
    extra_options = dict(
        ...
        options = dict(
            py2app = {
                ...
                'argv_emulation' : True,
                'includes' : ['sip', 'PyQt4.QtNetwork', 'mako.cache'],
                'plist' : {
    ...
    class custompy2app(py2app):
        def run(self):
            ...
            os.system('hdiutil convert -format UDCO -imagekey zlib-level=9 -o "%s" "%s"' % (dmgPath, dmgPathTemp))
...
# Windows
    ...
    # Find the directory within the "%SYSTEM%/WinSxS" folder where the
    # manifest file for the Visual C++ 9.0 DLLs are located, or die trying.
    dlldir = None
    winsxs = os.path.join(os.environ.get('WINDIR'), 'WinSxS')
    for rootdir, subdir, files in os.walk(winsxs):
        for file in files:
            if file == 'Microsoft.VC90.CRT.manifest':
                dlldir = rootdir
                break

    if not dlldir:
        print 'ERROR: No Visual C++ 9.0 DLLs found!'
        sys.exit(1)

    # Add each of those DLLs and the corresponding manifest file to the
    # distribution.
    data_files.append(('', glob.glob(os.path.join(dlldir, '*'))))
    data_files.append(('lib', glob.glob(os.path.join(dlldir, '*.manifest'))))
    ...
    extra_options = dict(
        ...
        options = {
            'py2exe' : {
                'bundle_files' : 2,
                'includes' : ['cuped', 'qt', \
                    'mako.cache', 'sip', 'PyQt4.QtNetwork'],
